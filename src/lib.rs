        ...
        syn::Expr::Lit(lit) =>
            match &lit.lit {
                ...
                syn::Lit::Int(_) | syn::Lit::Float(_) =>
                    lit.lit.to_token_stream().to_string().len(),
                _ => 0,
            }
        _ => 0,
    }
...
fn tmp_idents() -> impl Iterator<Item = syn::Ident> {
    (0..).into_iter().map(|n| { syn::Ident::new(&format!("__tmp_{}", n), Span::call_site()) })
}
...
fn impl_concat_strs(items: ConcatStrs) -> TokenStream {
    ...
    let tmp_assignments_idents = items.exprs
        .iter()
        .zip(tmp_idents())
    ...
    let tmp_len_idents = tmp_assignments_idents
        ...
        .filter_map(|(assign, _ident)| {
            if let Some((len_ident, _assign)) = assign { Some(len_ident) } else { None }
        })
        .collect::<Vec<_>>();
